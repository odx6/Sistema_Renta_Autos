                    ...
                    <div class="columna col-lg-12" >
                        ...
                        <div>
                            <table class="table" width="100%" style="background-color: #f7ff80;">
                                ...
                                <tbody>
                                    <?php foreach ($link->query("SELECT * FROM alquiler 
                                                                          WHERE ca LIKE  '%$busqueda%' 
                                                                          OR nm LIKE  '%$busqueda%' 
                                                                          OR dni LIKE  '%$busqueda%' 
                                                                          OR cr LIKE  '%$busqueda%' 
                                                                          OR fecha_i_a LIKE  '%$busqueda%'
                                                                          OR fecha_f_a LIKE  '%$busqueda%';") as $row) 
                                                                          { ?>
                                        <tr>
                                            <td><?php echo $row['ca'] ?></td>
                                            <td><?php echo $row['fecha_i_a'] ?></td>
                                            <td><?php echo $row['fecha_f_a'] ?></td>
